...
function articleCheck(fid, status) {
    ...
    var articletest=$('#form_listArticle_' + fid).serialize();
    //articletest.setAttribute("status",status);                      //设置属性
    //articletest.set
    $.ajax({
        url: APP_PATH + "/adminController/articleStatusByfid.do",
        type: "post",
        dataType: "json",
...
function articlePage(pageStart) {
    ...
    var adminList = document.getElementById("adminList").value;
    $.ajax({
        url: APP_PATH + "/adminController/getArticleManagement.do",
        type: "post",
        dataType: "json",
