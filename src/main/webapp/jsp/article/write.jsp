...
<div class="top-50 write-frame detail-center bottom-100">
  <form onsubmit="return check()" id="add-article" action="${APP_PATH}/article/addArticle.do" enctype="multipart/form-data" method="post">
    <input type="text" name="titles" class="form-control" placeholder="文章标题">
    ...
    <div style="height: 30px;"></div>
    <textarea name="fcontent" class="write-textarea" placeholder="文章内容"></textarea>
    <div style="height: 30px;"></div>
    ...
    <img id="img-choose" class="img-thumbnail" src="${APP_PATH}/static/images/icons/fatiePhoto.png" alt="">
    <div style="height: 30px;"></div>
...
<script>
    ...
    function check() {
        if($("input[name='titles']").val().length <= 0) {
            ...
            alert("文章长度超出限制");
            return false;
        } else if(!$("input[name='imgUrl']").get(0).files[0]) {
            alert("文章配图不能为空");
            return false;
        } else return true;
    }
