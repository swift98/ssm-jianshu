...
$(function () {
    ...
    $.ajax({
        url: APP_PATH+"/adminController/getAll_Admin.do",
        type: "post",
        dataType: "json",
        success: function (data) {
            ...
            for (var i=0;i<listUsers.length;i++){
                ...
                if(listUser["photo"] == null){  //默认头像
                    listUser_userphoto =
                        '<a href="'+APP_PATH+'/static/images/head.png" target="_brank">' +
                            '<img src="'+APP_PATH+'/static/images/head.png" style="position: relative; width: 100%; height: 100%;">' +
                        '</a>';
                } else {
                    listUser_userphoto =
                        '<a href="'+APP_PATH+'/static/images/user/'+listUser["photo"]+'" target="_brank">' +
                            '<img src="'+APP_PATH+'/static/images/user/'+listUser["photo"]+'" style="position: relative; width: 100%; height: 100%;">' +
                        '</a>';
                }
            ...
            for (var i=0;i<listArticles.length;i++){
                ...
                if (listArticle["photo"] != "photo"){   //有配图
                    ...
                    if (listArticle["photo"].endsWith(".mp4") || listArticle["photo"].endsWith(".avi")){    //视频
                        listArticle_photo_type =
                            '<video controls="controls" src="'+APP_PATH+'/static/images/article/'+listArticle["photo"]+'"' +
                                'style="position: relative; width: 100%; height: 100%;"></video>';
                    } else {
                        listArticle_photo_type =
                            '<img src="'+APP_PATH+'/static/images/article/'+listArticle["photo"]+'"' +
                                'style="position: relative; width: 100%; height: 100%;">';
                    }
                    listArticle_photo =
                        '<a href="'+APP_PATH+'/static/images/article/'+listArticle["photo"]+'" target="_brank" id="listArticle_photo_type">'+listArticle_photo_type+'</a>';
                }
                ...
                if (adminList == "" || adminList == null){  //未登录
                    ...
                    if (listArticle["status"] == 0){     //待审核
                        ...
                        /*
                        由于待审核的表单中有两个id是一样的，故修改两个表单的id，通过的加上pass
                         */
                        listArticle_caozuo =
